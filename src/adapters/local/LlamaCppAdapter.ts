...
export class LlamaCppAdapter extends BaseDiscussionAdapter {
  ...
  constructor(
    ...
    this.maxTokens = Math.max(64, Number(process.env.LLAMACPP_MAX_TOKENS || 1200));
  }
  ...
  protected async executeCompletion(
    ...
    if (!response.ok) {
      ...
      throw new Error(`Llama.cpp call failed (${response.status}): ${body}`);
    }
