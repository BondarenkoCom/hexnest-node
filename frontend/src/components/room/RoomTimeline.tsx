...
import { Clock, Bot, ShieldAlert, Cpu, History } from 'lucide-react';
...
export const RoomTimeline: React.FC<RoomTimelineProps> = ({ events }) => {
  ...
  const visibleEvents = sortedEvents.slice(-showCount);
  const hiddenCount = Math.max(0, sortedEvents.length - showCount);

  const handleShowMore = () => {
    setShowCount(prev => Math.min(sortedEvents.length, prev + 10));
  };
        ...
        <>
          {hiddenCount > 0 && (
            ...
            >
              ...
              Show previous {Math.min(10, hiddenCount)} ({hiddenCount} hidden)
            </button>
          )}
